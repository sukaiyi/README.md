 ...
 * @date 2020/08/12
 */
public class InterfaceBlock implements Block<Integer> {
    ...
    public Class<? extends Block> next(Map<Class<?>, List<Block<?>>> blockAlreadyDecode, List ret) {
        Integer interfaceCnt = (Integer) blockAlreadyDecode.get(InterfacesCountBlock.class).get(0).getDecodedData();
        Integer interfaceCntRead = blockAlreadyDecode.get(InterfaceBlock.class).size();
        return interfaceCnt > interfaceCntRead ? InterfaceBlock.class : FieldsCountBlock.class;
    }
}
